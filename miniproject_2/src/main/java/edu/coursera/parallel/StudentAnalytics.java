...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
        ...
        double result = Stream.of(studentArray)
                        .parallel()
                        .filter(v -> v.checkIsCurrent() == true)
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        //List<Student> Inactive = Stream.of(studentArray).parallel().filter(s -> s.checkIsCurrent() == false).toList();
        //HashMap<String, Integer> NamesAndHashes = 
        String result = Stream.of(studentArray).parallel()
                                    .filter(s -> s.checkIsCurrent() == false)
                                    .filter(s -> Objects.nonNull(s.getFirstName()))
